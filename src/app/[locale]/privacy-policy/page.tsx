...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  for (const l of locales) {
    alternates[l] = l === 'en' ? `${baseUrl}/privacy-policy` : `${baseUrl}/${l}/privacy-policy`
  }

  return {
    title: `${t('title')} — Sauditoolhub`,
    description: t('content.collect'),
    alternates: {
      canonical: pageUrl,
...
export default async function PrivacyPolicyPage({ params }: Props) {
  ...
  const sections = [
    { key: 'collect', titleKey: 'sections.collect' },
    { key: 'storage', titleKey: 'sections.storage' },
    { key: 'cookies', titleKey: 'sections.cookies' },
    { key: 'thirdParty', titleKey: 'sections.thirdParty' },
    { key: 'contact', titleKey: 'sections.contact' },
  ]
            ...
            <div className="space-y-6">
              {sections.map((section) => (
                <div key={section.key} className="glass p-6 rounded-xl">
                  ...
                  <p className="text-gray-300 text-sm leading-relaxed">{t(`content.${section.key}`)}</p>
                </div>
              ))}
            </div>
          </div>
